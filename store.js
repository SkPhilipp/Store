...
app.service("Store", ["$window", function ($window) {
	 ...
	 * @param storeKey is optional, defaults to given scopeKey
	 */
	this.mirror = function($scope, scopeKey, storeKey){

		storeKey = storeKey || scopeKey;
		...
		if($scope[scopeKey] == null && that.has(storeKey)){
			$scope[scopeKey] = that.get(storeKey);
		}

		// initialize Store when $store has no value and $scope does
		if(!that.has(storeKey) && $scope[scopeKey] == null){
			that.set(storeKey, $scope[scopeKey]);
		}

		// return either
		return $scope[scopeKey] || that.get(storeKey);
